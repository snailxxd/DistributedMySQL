...
public class Main {
    ...
    public static void buffer_unit_test() {
        ...
        try {
            BufferManager.initialBuffer(); // 必须先分配 buffer[0..49]，否则 readBlockFromDisk 会 NPE
            BufferManager.testInterface();
            int bid = BufferManager.readBlockFromDisk(buffer_test_file_name, 15);
            buffer_print(bid);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static void buffer_print(int bid) {
        System.out.println(bid);
        System.out.println("isLock = " + BufferManager.buffer[bid].lock());
        System.out.println("isDirty = " + BufferManager.buffer[bid].dirty());
        System.out.println("isValid = " + BufferManager.buffer[bid].valid());
        System.out.println(BufferManager.buffer[bid].readInteger(1200));
        System.out.println(BufferManager.buffer[bid].readFloat(76));
        System.out.println(BufferManager.buffer[bid].readString(492, 6));
        BufferManager.buffer[bid].writeInteger(128, -23333);
        System.out.println("isLock = " + BufferManager.buffer[bid].lock());
        System.out.println("isDirty = " + BufferManager.buffer[bid].dirty());
        System.out.println("isValid = " + BufferManager.buffer[bid].valid());
        System.out.println("LRUCnt = " + BufferManager.buffer[bid].getLRU());
        System.out.println(BufferManager.buffer[bid].readInteger(128));
    }
