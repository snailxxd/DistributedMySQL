...
public class Interpreter {
    ...
    public static String interpret(String sql) throws IOException {
        ...
        System.out.println("  [Interpreter] 【原始SQL】" + sql);
        String resultValue = "";

        String result = sql.trim().replaceAll("\\s+", " ");
        System.out.println("  [Interpreter] 【规范化后】" + result);

        String[] tokens = result.split(" ");
        System.out.println("  [Interpreter] 【操作类型】" + tokens[0]);
        System.out.println("  [Interpreter] 【Token数量】" + tokens.length);
        ...
        try {
            ...
            switch (tokens[0]) { //match keyword
                case "create":
                    ...
                    System.out.println("  [Interpreter] 【子操作】CREATE " + tokens[1].toUpperCase());
                    switch (tokens[1]) {
                        case "table":
                ...
                case "drop":
                    ...
                    System.out.println("  [Interpreter] 【子操作】DROP " + tokens[1].toUpperCase());
                    switch (tokens[1]) {
                        case "table":
                    ...
                    break;
                case "select":
                    System.out.println("  [Interpreter] 【子操作】SELECT");
                    resultValue += parseSelect(result);
                    break;
                case "insert":
                    System.out.println("  [Interpreter] 【子操作】INSERT");
                    resultValue += parseInsert(result);
                    break;
                case "delete":
                    System.out.println("  [Interpreter] 【子操作】DELETE");
                    resultValue += parseDelete(result);
                    break;
                case "execfile":
                    System.out.println("  [Interpreter] 【子操作】EXECFILE");
                    parseSqlFile(result);
                    break;
                case "show":
                    System.out.println("  [Interpreter] 【子操作】SHOW");
                    parseShow(result);
                    break;
                default:
            ...
            System.out.println("  [Interpreter] ❌ 【错误】" + e.status + " - " + QException.ex[e.type] + ": " + e.msg);
        } catch (Exception e) {
            System.out.println("  [Interpreter] ❌ 【异常】" + e.getClass().getSimpleName() + ": " + e.getMessage());
            e.printStackTrace();
        }
        System.out.println("  [Interpreter] 【最终结果】" + (resultValue.length() > 100 ? resultValue.substring(0, 100) + "..." : resultValue));
        return resultValue;
    }
